...
from numba import int32, float32, boolean, jit

from numba import types

import newService
import config
import numba_main.BS_approach as get_T_eff
...
Surface_spec = [
    ('surf_R_e', float32),
    ('surface_type', types.unicode_type),
    ('theta_accretion_begin', float32),
    ('theta_accretion_end', float32),
    ('theta_range', float32[:]),
    ('phi_range', float32[:]),
    ('mask_array', boolean[:, :]),
    ('array_normal', float32[:, :]),
]


@jitclass(Surface_spec)
class Surface:
    ...
    def __init__(self, surf_R_e, col_R_e, beta_mu, a_portion, phi_0, ksi_shock, column_type, surface_type):
        ...
        self.mask_array = np.zeros((config.N_phi_accretion, config.N_theta_accretion), dtype=np.bool_)
    ...
    def create_array_normal(self, phi_range, theta_range, surface_type):
        ...
        coefficient = -1
        if surface_type == 'outer':  # True - внешняя поверхность, False - внутренняя
            coefficient = 1
...
MagnetLine_spec = [
    ('column_type', types.unicode_type),
    ('surf_R_e', float32),
    ('theta_range', float32[:]),
    ('phi_range', float32[:]),
    ('mask_array', boolean[:, :]),
...
surface_column_type = Surface.class_type.instance_type
AccretionColumn_spec = [
    ('R_e', float32),
    ('column_type', types.unicode_type),
    ('a_portion', float32),
    ('phi_0', int32),
    ('R_e_outer_surface', float32),
    ('R_e_inner_surface', float32),
    ('T_eff', float32[:]),
    ('ksi_shock', float32),
    ('L_x', float32),
    ('beta', float32),
    ('outer_surface', surface_column_type),
    ('inner_surface', surface_column_type),
]


@jitclass(AccretionColumn_spec)
class AccretionColumn:
    ...
    def __init__(self, R_e, mu, beta_mu, mc2, a_portion, phi_0, column_type):
        ...
        self.outer_surface = Surface(self.R_e_outer_surface, self.R_e, beta_mu, self.a_portion, self.phi_0,
                                     self.ksi_shock, self.column_type, surface_type='outer')
        self.inner_surface = Surface(self.R_e_inner_surface, self.R_e, beta_mu, self.a_portion, self.phi_0,
                                     self.ksi_shock, self.column_type, surface_type='inner')
...
accretion_column_type = AccretionColumn.class_type.instance_type
magnet_lines_type = MagnetLine.class_type.instance_type
AccretingPulsarConfiguration_spec = [
    ('mu', float32),
    ('theta_obs', int32),
    ('beta_mu', int32),
    ('mc2', int32),
    ('a_portion', float32),
    ('phi_0', int32),
    ('M_accretion_rate', float32),  # ??
    ('R_alfven', float32),
    ('R_e', float32),
    ('top_column', accretion_column_type),
    ('bot_column', accretion_column_type),
    ('top_magnet_lines', int32),
    ('bot_magnet_lines', int32),
]


@jitclass(AccretingPulsarConfiguration_spec)
