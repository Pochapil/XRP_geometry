            ...
            for mc2 in mc2_arr:
                for a_portion in a_portion_arr:
                    for phi_0 in phi_0_arr:
                        curr_configuration = accretingNS.AccretingPulsarConfiguration(mu, beta_mu, mc2, a_portion,
                                                                                      phi_0)
                        ...
                        for i, surface in enumerate(accr_col_surfs):
                            ...
                            L_surfs[i] = integralsService.calc_L(surface, curr_configuration.top_column.T_eff,
                                                                 new_cos_psi_range)
                            L_nu_surfs[i] = integralsService.calc_L_nu(surface, curr_configuration.top_column.T_eff,
                                                                       new_cos_psi_range)
                        ...
                        for i, magnet_surface in enumerate(magnet_line_surfs):
                            ...
                            xyz_magnet_line = matrix.get_xyz_coord(magnet_surface,
                                                                   normalize=True)  # вектор на отражающую площадку
                            ...
                            L_x = integralsService.calculate_total_luminosity(
                                curr_configuration.top_column.inner_surface,
                                curr_configuration.top_column.T_eff)
                            # надо брать не curr_configuration.top_column.L_x а посчитанный через интеграл! хоть они и отличаются на сильно
                            L_scatter[i] = integralsService.calc_scatter_L(magnet_surface, L_x, new_cos_psi_range,
                                                                           tau_scatter_matrix)
                        ...
                        # plot_package.plot_scripts.plot_Teff_to_ksi(curr_configuration.R_e,
                        #                                            curr_configuration.top_column.T_eff,
